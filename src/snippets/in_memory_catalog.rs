...
use crate::snippets::{SnippetCatalog, SnippetEntry};
...
impl SnippetCatalog for InMemoryCatalog {
    ...
    fn resolve_snippet(&self, raw_query: &str) -> Result<SnippetEntry, AppError> {
        ...
        if key_match.len() > 1 {
            return Err(AppError::ConfigError(crate::error::ConfigError::Other(format!(
                "Multiple snippets share the name '{raw_query}'",
            ))));
        }

        Err(AppError::NotFound(crate::error::NotFoundError::Snippet(format!(
