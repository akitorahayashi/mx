...
use crate::snippets::{SnippetFrontmatter, SnippetStore};
...
mod tests {
    ...
    fn rejects_duplicate_without_force() {
        ...
        assert!(matches!(
            err,
            AppError::ConfigError(crate::error::ConfigError::DuplicateSnippet(_))
        ));
    }
    ...
    fn rejects_path_outside_mx_commands() {
        ...
        assert!(matches!(
            err,
            AppError::InvalidKey(crate::error::InvalidKeyError::NotInCommands { .. })
        ));
    }
}
